...
def load_data():
    try:
        ...
        df = pd.read_sql(
            "SELECT * FROM sentiment_results ORDER BY published_at DESC LIMIT 500",
            con
        )
        con.close()
...
if "ensemble_uncertainty" in df.columns:
    ...
    fig = px.scatter(
        df, x="ensemble_confidence", y="ensemble_uncertainty",
...
show_cols = [c for c in [
    "headline", "source", "ensemble_sentiment", "ensemble_confidence",
    "ensemble_uncertainty", "market_impact", "tickers", "is_uncertain"
] if c in df.columns]
